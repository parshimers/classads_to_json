...
loop:
	...
	if (cptr[1] == '<')
	{
		...
		if (jflag)
		    fprintf(f, "((%s)valStack[stackTop - %d])", tag, n - i);
		else
		...
		i = -get_number() - n;
		if (jflag)
		    fprintf(f, "((%s)valStack[stackTop - %d])", tag, i);
		else
	...
	else if (isdigit((unsigned char)cptr[1]))
	{
		...
		if (jflag)
		    fprintf(f, "valStack[stackTop - %d]", n - i);
		else
	...
	else if (cptr[1] == '-')
	{
		...
		unknown_rhs(-i);
	    if (jflag)
		fprintf(f, "valStack[stackTop - %d]", i+n);
	    else
		fprintf(f, "yyvsp[%d]", -i - n);
	    goto loop;
	}
...
void reader(void)
{
    write_section(jflag ? jbanner : banner);
    create_symbol_table();
    read_declarations();
    read_grammar();
    free_symbol_table();
