...
void union_ignored(char *u_cptr)
{
    fprintf(stderr, "%s: e - line %d of \"%s\", %%union \
declarations are ignored with Java\n", myname, lineno, input_file_name);
    print_pos(line, u_cptr);
}
...
void typed_lhs_warning(void)
{
    fprintf(stderr, "%s: e - line %d of \"%s\", "
	    "$<type>$ is not supported in Java\n",
	    myname, lineno, input_file_name);
}
