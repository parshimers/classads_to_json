...
{
    if (!jflag && strcmp(symbol_prefix, "yy")!=0)
    {
	++outline;
...
void output_rule_data(void)
{
    ...
    fprintf(output_file, "%sshort %slhs[] = {%42d,",
	    const_array, symbol_prefix, symbol_value[start_symbol]);
    ...
    fprintf(output_file, "%sshort %slen[] = {%42d,",
	const_array, symbol_prefix, 2);
...
void output_yydefred(void)
{
    ...
    fprintf(output_file, "%sshort %sdefred[] = {%39d,",
	const_array, symbol_prefix, (defred[0] ? defred[0] - 2 : 0));
...
void goto_actions(void)
{
    ...
    fprintf(output_file, "%sshort %sdgoto[] = {%40d,",
	const_array, symbol_prefix, k);
...
void output_base(void)
{
    ...
    fprintf(output_file, "%sshort %ssindex[] = {%39d,",
	const_array, symbol_prefix, base[0]);
    ...
    if (!rflag) outline += 2;
    fprintf(output_file, "\n};\n%sshort %srindex[] = {%39d,",
	const_array, symbol_prefix, base[nstates]);
    ...
    if (!rflag) outline += 2;
    fprintf(output_file, "\n};\n%sshort %sgindex[] = {%39d,",
	const_array, symbol_prefix, base[2*nstates]);
...
void output_table(void)
{
    ...
    fprintf(output_file, "%sshort %stable[] = {%40d,",
	const_array, symbol_prefix, table[0]);
...
void output_check(void)
{
    ...
    fprintf(output_file, "%sshort %scheck[] = {%40d,",
	const_array, symbol_prefix, check[0]);
...
void output_defines(void)
{
    ...
    ++outline;
    fprintf(code_file,
	jflag ? "private final static short ERROR_TOKEN = %d;\n"
	    : "#define YYERRCODE %d\n", symbol_value[1]);
...
void output_debug(void)
{
    int i, j, k, max;
    char **symnam, *s;
    char *missing = jflag ? "null," : "0,";

    ++outline;
    fprintf(code_file,
	jflag ? "private final static short FINAL_STATE = %d;\n"
	    : "#define YYFINAL %d\n",
	final_state);
    outline += 3;
    if (!jflag)
    ...
    fprintf(output_file, "%s%s%sname[] = {",
	const_array, jflag ? "String " : "char *", symbol_prefix);
    j = 80;
    for (i = 0; i <= max; ++i)
    {
	if ((s = symnam[i]))
    ...
    if (jflag)
    {
	fprintf(output_file, "private final static String ntname[] = {\n");
	for (i = start_symbol + 1; i < nsyms; i++)
	{
	    fprintf(output_file, "    \"%s\"%s\n",
		symbol_name[i], (i < nsyms-1) ? "," : "");
	}
	fprintf(output_file, "};\n");
    }
}
